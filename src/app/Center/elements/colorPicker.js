...
const ColorPicker = ({ pickedColor }) => {
  ...
  return (
    <div className="w-full mb-4">
      {/* Cím méretezése: text-base mobilra, sm:text-lg asztalira */}
      <p className="text-base sm:text-lg font-semibold mb-2">Color Selection</p>
        ...
        `}>
        {
          colors.map((colorItem) => (
            <div
              ...
              className={`
                /* Színnégyzet méretezése: w-8 h-8 mobilra, sm:w-11 sm:h-11 asztalira */
                w-8 h-8 xl:w-6 xl:h-6 xl:rounded-sm rounded-md duration-200 hover:scale-110 cursor-pointer 
